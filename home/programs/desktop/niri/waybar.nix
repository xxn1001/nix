...
let
  ...
  # 修复颜色函数映射逻辑
  colorNames = [ "base00" "base01" "base04" "base05" "base06" "base07" "base08" "base0A" "base0B" "base0D" "base0E" "base0F" ];
in
{
  programs.waybar = {
    enable = true;
  ...
  # 禁用 Stylix 的 Waybar 自动生成以防止 CSS 冲突
  stylix.targets.waybar.enable = false;

  home.file = {
    ".config/waybar/config.jsonc".text =
      # 适配虚拟机 Virtual-1 并保留主显示器逻辑
      ''
        [
          {
            "position": "top",
            "layer": "top",
            "output": ["Virtual-1", "${config.lib.monitors.mainMonitorName}", "*"],
            "modules-left": [
              "niri/workspaces",
              "tray",
              "niri/window"
    ...
    ".config/waybar/colors.css".text =
      builtins.concatStringsSep "\n" (
        map (name: "@define-color ${name} ${withHashtag.${name}};") colorNames
      );
    ...
    ".config/waybar/style.css".text =
      ''
        ...
        * {
          font-size: 14px;
          font-family: "Maple Mono NF CN", "Xiaolai Mono SC", sans-serif;
          min-height: 0;
        }
        ...
        #battery {
          ...
          background: linear-gradient(
            118deg, @base0B 5%, @base0F 5%, @base0F 20%, @base0B 20%, @base0B 40%, @base0F 40%, @base0F 60%, @base0B 60%, @base0B 80%, @base0F 80%, @base0F 95%, @base0B 95%
          );
        ...
        #battery.charging, #battery.plugged {
          background: linear-gradient(
            118deg, @base0E 5%, @base0D 5%, @base0D 20%, @base0E 20%, @base0E 40%, @base0D 40%, @base0D 60%, @base0E 60%, @base0E 80%, @base0D 80%, @base0D 95%, @base0E 95%
          );
    ...
    ".config/waybar/animation.css".text =
      ''
        @keyframes gradient_f { 0% { background-position: 0% 200%; } 50% { background-position: 200% 0%; } 100% { background-position: 400% 200%; } }
        @keyframes gradient_f_nh { 0% { background-position: 0% 200%; } 100% { background-position: 200% 200%; } }
        @keyframes gradient_rv { 0% { background-position: 200% 200%; } 100% { background-position: 0% 200%; } }
      '';
  };
}
